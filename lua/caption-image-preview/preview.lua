...
local state = {
    ...
    updating = false,
    update_timer = nil,
    last_file = nil,  -- Track last file to avoid unnecessary updates
}
...
local function render_preview(buf, win, image_path)
    ...
    if img then
        ...
        vim.schedule(function()
            ...
            if state.active and state.win == win and state.buf == buf and
               vim.api.nvim_win_is_valid(win) and vim.api.nvim_buf_is_valid(buf) then
                pcall(function()
                    img:render()
                end)
            else
                -- Stale render: clean up
                pcall(function()
                    img:clear()
                end)
                -- CRITICAL: Clear buffer for stale renders
...
function M.toggle()
    ...
    vim.api.nvim_set_option_value('modifiable', true, { buf = preview_buf })  -- Keep modifiable
    ...
    -- Keymaps
    local toggle_cmd = ':lua require("caption-image-preview.preview").toggle()<CR>'
    vim.api.nvim_buf_set_keymap(preview_buf, 'n', 'q', toggle_cmd, {
        silent = true,
        noremap = true
    })
    vim.api.nvim_buf_set_keymap(preview_buf, 'n', '<Esc>', toggle_cmd, {
        silent = true,
        noremap = true
    })
